...
const std::vector<SettingDefinition> kCatalog = {
    ...
    Choice(
        ...
        "Native preserves the Quest 3 eye and scope layout. Performance uses the verified lower mode. Pimax Full FOV fits both uncropped 3450 x 4082 eyes plus the 1024 px scope panel; the cropped-FOV mode remains available for Pimax Play FOV crop users.",
        SettingPage::Graphics,
        "6016x2688",
        {{"6016x2688", "Native 6016 x 2688"}, {"4768x2016", "Performance 4768 x 2016"}, {"7924x4082", "Pimax Full FOV 7924 x 4082"}, {"7684x3128", "Pimax Cropped FOV 7684 x 3128"}}),
    Choice(
        ...
        SettingPage::Graphics,
        "1.00",
        {{"1.00", "Native 1.00"}, {"0.80", "Pimax Full FOV 0.80"}, {"0.75", "Performance 0.75"}}),
    Toggle(
        "KISAK_VR_FSR",
        "FSR upscaling",
    ...
    const SettingsMap& values)
{
    ...
    if (valueOf("VR_CUSTOM_MODE") == "7684x3128" &&
        valueOf("KISAK_VR_OUTPUT_SCALE") != "1.00")
    {
        messages.push_back({
            ValidationMessage::Severity::Error,
            "KISAK_VR_OUTPUT_SCALE",
            "Pimax cropped-FOV packed mode requires an eye output scale of exactly 1.00.",
        });
    }

    // KISAK_SP_VR_PIMAX_FULL_FOV_SCOPE_LAYOUT_V89
    // Pimax Play with FOV crop disabled reports 4312 x 5102 per eye at 1.00.
    // Scaling those rectangular eyes to 0.80 yields 3450 x 4082, so the
    // complete packed surface is 3450 + 3450 + 1024 = 7924 pixels wide.
    if (valueOf("VR_CUSTOM_MODE") == "7924x4082" &&
        valueOf("KISAK_VR_OUTPUT_SCALE") != "0.80")
    {
        messages.push_back({
            ValidationMessage::Severity::Error,
            "KISAK_VR_OUTPUT_SCALE",
            "Pimax full-FOV packed mode requires an eye output scale of exactly 0.80.",
        });
    }

    double scopeCaptureSize = 0.0;
    if ((valueOf("VR_CUSTOM_MODE") == "7684x3128" ||
         valueOf("VR_CUSTOM_MODE") == "7924x4082") &&
        ParseFiniteNumber(
            valueOf("KISAK_VR_SCOPE_CAPTURE_SIZE"),
            &scopeCaptureSize) &&
        scopeCaptureSize > 1024.0)
    {
        messages.push_back({
            ValidationMessage::Severity::Error,
            "KISAK_VR_SCOPE_CAPTURE_SIZE",
            "Pimax Crystal Light packed modes reserve a maximum 1024 px dedicated scope panel.",
        });
    }
    ...
    SettingsMap* values)
{
    ...
    if (presetName == "Pimax Crystal Light")
    {
        Set(values, "VR_CUSTOM_MODE", "7924x4082");
        Set(values, "KISAK_VR_OUTPUT_SCALE", "0.80");
        Set(values, "KISAK_VR_FSR", "0");
        Set(values, "KISAK_VR_SCOPE_CAPTURE_SIZE", "1024");
        return true;
    }
